...
const ChatBox = () => {
  ...
  const onSubmit = async (e) => {
    ...
    const startTypingAnimation = async () => {
      try {
        const { content: fullContent, charts } = await generateResponse(trimmedPrompt)
  ...
  return (
    <div className='flex-1 flex flex-col justify-between p-5 md:p-10 max-md:pt-20 2xl:pr-40 h-full overflow-hidden bg-white dark:bg-transparent'>
      ...
      {uploadSuccess && (
        <div className={`mt-2 mx-auto max-w-2xl lg:max-w-3xl px-2 py-2 rounded-lg text-center text-sm font-medium transition-opacity duration-300 ${
          theme === 'dark'
